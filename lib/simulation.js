...
var iterate = function(iters, nCands, nJudgements, algorithm, thr, AP, callback){
  var results =[];
  series(iters, nCands, nJudgements, algorithm, thr, AP, results, function(results){
    csv
...
var series = function(iter, nCands, nJudgements, algorithm, thr, AP, iterResults, callback) {
  if(iter>0) {
    simulate(nCands, nJudgements, algorithm, thr, AP, function(results) {
      iter --;
      for(var i=0; i<results.length; i++){
        var pl ={};
        pl.iter = iter+1;
        pl.judgement = (nJudgements - results[i].judgement) + 1;
        pl._id = results[i]._id;
        pl.observedScore = results[i].observedScore;
        pl.comparisons = results[i].comparisons;
        pl.trueScore = results[i].trueScore;
        pl.theta = results[i].theta;
        iterResults.push(pl);
      }
      series(iter,nCands, nJudgements, algorithm, thr, AP, iterResults, callback);
    });
  } else {
    callback(iterResults);
  }
};

var simulate = function(nCands, nJudgements, algorithm, thr, AP, callback){
	// Give me some candidates with a theta and true score of zero
  var players = chooseCandidates(nCands);
  // Do judgements until nJudgements has been reached
  doJudgements(algorithm, players, thr, AP, nJudgements, [], function(results){
      callback(results);
  });
};
...
var doJudgements = function(algorithm, players, thr, AP, mx, results, callback){
  ...
  doJudgement(algorithm, players, thr, AP, function(players){
    ...
    estimateCJ('1',players,function(task, players){
      ...
      mx--;
      if(mx>0){
        doJudgements(algorithm, players, thr, AP, mx, results, callback);
      } else {
        callback(results);
      }
    });
