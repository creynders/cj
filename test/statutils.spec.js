...
describe('statutils', function () {
  ...
  describe('iI', function () {
    var iI = statutils.iI;
    var ii = iI(0, [0, 0]);
    expect(ii).eql([0.25, 0.25]);
    ii = iI(0, [-5, -4.5, -4]);
    ii = _.map(ii, function (i) {
      return (+i.toFixed(4));
    });
    ...
    ii = iI(4.15, [5.05, 5.05, -4.15, -4.15, -5.05]);
    ii = _.map(ii, function (i) {
      return (+i.toFixed(4));
    });
