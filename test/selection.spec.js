    ...
    describe('selectionAdaptive',function(){
        it('should always return player with fewest comparisons', function(){
          ...
          for(var i=0; i<100; i++){
            var selected = Math.floor(Math.random() * 100) + 1;
            var mn = (selected < mn ? selected : mn);
            pls.push({selected:selected, trueScore:0, comparisons:selected});
          }
        ...
        it('with 0 judgements it should partner with next fewest judgements, not itself', function(){
          var pls = [];
          for(var i = 0; i<2; i++){
            pls.push({_id:i, selected:0, comparisons:0, trueScore:0});
          }
          for(var i = 0; i<50; i++){
            pls.push({_id:i, selected:1, comparisons:0, trueScore:0});
          }
          for(var i=0; i<100; i++){
            var pair = selection.selectionAdaptive(pls);
        ...
        it('should return partner closer to ability more often than not', function(){
          ...
          for(var i = 0; i<5; i++){
            pls.push({_id:i, selected:i+1, comparisons:i+1, trueScore:i});
          }
          for (var i=0; i<100; i++){
            var pair = selection.selectionAdaptive(pls);
